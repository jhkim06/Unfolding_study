...
class TUnfoldBinningV17 : public TNamed {
 protected:
   TUnfoldBinningV17 *parentNode; // mother node
   TUnfoldBinningV17 *childNode; // first daughter node
   TUnfoldBinningV17 *nextNode; // next sister
   TUnfoldBinningV17 *prevNode; // previous sister
   ...
   TObject *fBinFactorFunction; // function to calculate user factor from bin centres (may be a TF1 or a TVectorD
 ...
 public:
   ...
   TUnfoldBinningV17 *AddBinning(TUnfoldBinningV17 *binning); // add a new node to the TUnfoldBinning tree
   ...
   /********************* Navigation **********************/
   inline TUnfoldBinningV17 const *GetChildNode(void) const { return childNode; } // first daughter
   inline TUnfoldBinningV17 const *GetPrevNode(void) const { return prevNode; } // previoous sister
   inline TUnfoldBinningV17 const *GetNextNode(void) const { return nextNode; } // next sister
   inline TUnfoldBinningV17 const *GetParentNode(void) const { return parentNode; } // mother
   ...
   Int_t FillBinMap1D(Int_t *binMap,const char *axisSteering,
                     TH1 *destHist,Int_t firstBinX) const; // map bins from this distribution to destHist
   ...
   Int_t GetGlobalBinNumber(const Double_t *x,Int_t *isBelow=0,Int_t *isAbove=0) const; // get bin number, up to 32 dimenstional binning 
   ...
   Bool_t HasUnderflow(int axis) const { return fHasUnderflow & (1<<axis); } // check whether the axis has an underflow bin
   Bool_t HasOverflow(int axis) const { return fHasOverflow & (1<<axis); } // check whether the axis has an overflow bin
   void DecodeAxisSteering(const char *axisSteering,const char *options,
			     Int_t *isOptionGiven) const; // decode axis steering options
 protected:
   TUnfoldBinningV17 *GetRootNode(void); // return root node
   TUnfoldBinningV17 const *GetRootNode(void) const; // return root node
   ...
   const TUnfoldBinningV17 *GetNonemptyNode(void) const; // get the only nodes with non-empty distributions if there are multiple nodes, return 0
