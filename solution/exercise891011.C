...
void exercise891011(void) {
  //==============================================================
  // Unfolding exercise 3,7,8,9,10,11
...
#define NITERATION 100
  ...
  TH1D *hist_data_TUNF0=new TH1D("hist_data_TUNF0",
				 ...
				 nBin,r0,r1);
  // regularized unfolding (L-curve scan
  TH1D *hist_data_LCURVE=new TH1D("hist_data_LCURVE",
				  "L-curve scan (TUnfold) nRec=100;r_{fit}",
				 nBin,r0,r1);
  ...
  unfold.SetInput(hist_data_recTUNF0,1.0);
  unfold.DoUnfold(0.0);
  unfold.GetOutput(hist_data_TUNF0);
  TGraph *lCurve=0;
  unfold.ScanLcurve(50,0.,0.,&lCurve);
  ...
  hist_data_truth->Draw("HIST SAME");
  mc1_truth->Draw("HIST SAME");
  TLegend *legend6=new TLegend(0.25,0.7,0.85,0.9);
  legend6->SetBorderSize(0);
  legend6->SetFillStyle(0);
  legend6->AddEntry(hist_data_LCURVE,
		    TString::Format("data TUnfold #tau=%6.1e",tau),"l");
  legend6->AddEntry(hist_data_truth,"data truth","l");
  legend6->AddEntry(mc1_truth,"MC1 truth","pl");
  legend6->Draw();
